...
- Source and issue tracker: public GitHub repository

## Required description

CoreTune is a client-side Fabric optimization that limits how many new particles enter Minecraft's particle engine during one client tick. It can reduce rendering spikes during extreme particle storms. It starts disabled, uses a configurable default limit of 300 particles per tick, changes only local particle rendering, and does not modify packets, movement, combat, inventory, targeting, or clicks.

The listing must also state:

- F6 enables or disables the optimization.
- The configuration file is `config/coretune.properties`.
...
Do not advertise CoreTune as an anti-cheat bypass, a DonutSMP-approved mod, a competitive advantage, or a guaranteed FPS increase.
...
CoreTune appears compatible with Modrinth's content rules because its disclosed function is a narrow performance limit and it contains none of the specifically prohibited cheat functions. Moderators make the final decision; do not alter or obscure the description to influence review.
...
DonutSMP's published rules prohibit unauthorized modifications that provide unfair advantages and describe real-time client-integrity checks. CoreTune does not contain the listed movement, inventory, health, radar, automation, or packet features, but its particle suppression is still a client modification.

Therefore:

- Do not claim that CoreTune is allowed on DonutSMP.
