...
## Fabric 26.3 separate release (v1.2.5 draft)

The Fabric 26.3 port ships on its own release train. Keep `mod_version=1.2.5` and use the separate annotated tag `v1.2.5-mc26.3` only after the release PR is merged and the required checks are reviewed. `.github/workflows/release-26.3.yml` builds only this Fabric target and creates a **draft**, non-latest GitHub Release containing:
...
- Modrinth: use the exact five files above and target-specific version identifiers such as `1.2.5+mc1.21.1-fabric` and `1.2.5+mc26.2-neoforge`.
- CurseForge: tag 1.21.1 Fabric, 1.21.1 NeoForge, 1.21.11 Fabric, 26.2 Fabric, and 26.2 NeoForge separately, then upload the corresponding exact file.
