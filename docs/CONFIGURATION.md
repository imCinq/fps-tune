...
| `weatherMode` | `VANILLA` | `VANILLA`, `REDUCED` or `OFF`. `REDUCED` draws rain and snow in a smaller circle around you and skips every other landing splash; `OFF` suppresses local rain and snow rendering. Both apply only while FPS Tune is active. |
...
When `enabled=true` and `weatherMode=REDUCED`, FPS Tune keeps only the precipitation columns within three quarters of vanilla's weather radius (rounded up, at least three blocks) and skips every other landing splash particle. On Minecraft 1.21.1, which is not part of the current release, `REDUCED` renders like `VANILLA`.
